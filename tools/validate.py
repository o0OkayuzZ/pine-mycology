...
NF=load(ROOT/'data/nether_fungi.json')['entries']
...
ok('Texture index has classic + NF keys and field guide',set(T)=={e['textureKey'] for e in E}|{e['textureKey'] for e in NF}|{'pinene_myco_field_guide'})
for e in E:
...
ok('100 Nether fungi registry entries',len(NF)==100)
ok('50 crimson, 50 warped',sum(e['group']=='crimson' for e in NF)==50 and sum(e['group']=='warped' for e in NF)==50)
for field in ['id','itemId','textureKey']:ok('Unique NF '+field,len({e[field] for e in NF})==100)
for e in NF:
 p=RP/(e['texturePath']+'.png');im=Image.open(p)
 ok(e['id']+' NF is a real 32x32 RGBA PNG',im.size==(32,32) and im.mode=='RGBA')
 item_file=e['itemId'].split(':',1)[1]+'.json'
 item=load(BP/f'items/nether_fungi/{item_file}')['minecraft:item'];c=item['components']
 ok(e['id']+' NF item/texture linkage',item['description']['identifier']==e['itemId'] and c['minecraft:icon']['textures']['default']==e['textureKey'] and T[e['textureKey']]['textures']==e['texturePath'])
 ok(e['id']+' NF edible component',c.get('pinene:myco_consume')=={} and 'minecraft:food' in c)
 ok(e['id']+' NF shared cooldown',c['minecraft:cooldown']=={'category':'pinene_mushroom','duration':4})
...
report=[];passed=0
for command in [['node','--test','tests/core.test.mjs','tests/nether-fungi.test.mjs'],['node','--experimental-loader','./tests/mock-loader.mjs','--test','tests/adapters.test.mjs'],['node','--experimental-loader','./tests/mock-loader.mjs','--test','tests/field-guide.test.mjs'],['node','--experimental-loader','./tests/mock-loader.mjs','--test','tests/ui.test.mjs'],['node','--experimental-loader','./tests/mock-loader.mjs','--test','tests/reveal.test.mjs']]:
...
r=subprocess.run(['node','--input-type=module','-e',"import {NETHER_FUNGI} from './pack/BP/scripts/mycology/nether_registry.js';console.log(JSON.stringify(NETHER_FUNGI));"],cwd=ROOT,capture_output=True,text=True,check=True)
nf_runtime=json.loads(r.stdout)
for a,b in zip(NF,nf_runtime):
 for k,v in b.items():ok('Generated NF registry matches '+a['id']+' '+k,a[k]==v)
ok('No forgotten NF runtime entries',len(nf_runtime)==len(NF))
